...
from dtw_live.dtw import (
    similarity_cost_matrix,
    warp_path,
...
class OneVsRestStreamClassifier(
    BaseEstimator, ClassifierMixin, MultiOutputMixin, ModelBase
):
    ...
    psi : float or int, default=0
        ...
        absolute value (int).
    use_mp : boolean, default=True
        Use multiprocessing to compute DTW costs.
    TODO: groups : nested list with shape (n_groups,), default=None
    ...
    TODO: window_size : float or int, default=1.0
    ...
    def fit(self, X, y):
        ...
        Returns
        -------
        OneVsRestStreamClassifier object
            The fitted classifier.
        """
        X = check_array(X, allow_nd=True, force_all_finite=False)
        X, y = check_X_y(X, y, allow_nd=True, multi_output=True, force_all_finite=False)
        ...
        # print([f"{str(target_names[t])}: {a.shape[0]}" for a, t in zip(X, y)])
        ...
        if self.metric == "dtw":
            mats, ix = similarity_cost_matrix(X, psi=self.psi, use_mp=self.use_mp)
            if isinstance(self.psi, tuple):
                raise ValueError("unbalanced psi values not allowed for fit")
            elif self.psi > 0.0:
                # trim sequences based on warping path start/end points
                paths = [warp_path(m, psi=self.psi) for m in mats]
                ...
                X = [
                    x[ts : te + 1, :] if (te - ts) > (1 - self.psi) * x.shape[0] else x
        ...
        if self.target_names is not None:
            self.debug_dists = [
                {self.target_names[k]: v for k, v in d.items()} for d in target_dists
            ]
    ...
    def predict(self, X):
        ...
        for data in X:
            ...
            mats, _ = stream_similarity_cost_matrix(
                data, self.X_fit, psi=self.psi, use_mp=self.use_mp
            )
    ...
    def score(self, X, y, sample_weight=None):
        ...
        for data, targets in zip(X, y):
            ...
            for true_label, ts, te in ground:
                flag = False

                for i, (predict_label, t0, t1) in enumerate(events):
                    # event within bounds (P)
                    if ts < t0 <= te or ts < t1 <= te:
                        if true_label == predict_label:  # correct class (T)
                            if flag:
                                TPR += 1  # repeated
                            else:
                                TP += 1
                        ...
                        flag = True
                        events.pop(i)
                        self.conf_mat[true_label + 1][predict_label + 1] += 1

                if not flag:  # no event found within bounds (FN)
                    FN += 1
                    self.conf_mat[true_label + 1][0] += 1
            ...
            for (
                ...
                _,
                _,
            ) in events:  # add remaining events to null class (FP)
                self.conf_mat[0][predict_label + 1] += 1
    ...
    def _predict_frame_costs(self, costs):
        ...
        if e_durs and all(e_durs):
            ...
            # NOTE: first event is picked in the event of multiple choices
            counts = Counter(et).most_common(len(self.classes_))
            modes = [label for label, c in counts if c == counts[0][1]]
            e = modes[0]
