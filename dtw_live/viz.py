...
# cmap = 'viridis'
cmap = ListedColormap(
    [  # Multi-hue (exported from ColorBrewer):
        "#EDF8FB",
        "#BFD3E6",
        "#9EBCDA",
        "#8C96C6",
...
class DtwAxes(plt.Axes):
    ...
    def boundary(
        self, *args, labels=None, color="red", label_color="black", ls="dashed", lw=0.6
    ):
        for i, a in enumerate(args):
            ...
            if self.vertical:
                ...
                if labels:
                    super().text(
                        0.15 * (self.get_xlim()[0] + self.get_xlim()[1]),
                        a[0],
                        labels[i],
                        color=label_color,
...
def cost_matrix(
    mat, path=None, s1=None, l1=None, s2=None, l2=None, plot_trim=False, **kwargs
):
...
def class_distances(dists_dicts, thresholds=None, **kwargs):
    ...
    fig = plt.figure(figsize=kwargs.get("figsize", (4, 5)), constrained_layout=True)
    # shared_class = all(dists_dicts[0][0] == a[0] for a in dists_dicts[1:])
    ...
    for i, (l, d) in enumerate(dists_dicts):
        ...
        # vidx = np.where(np.array(xlabels) == l)[0]
        # ax.axvspan(vidx - 0.35, vidx + 0.35, color='green', alpha=0.2)
        # ax.boxplot(d.values())
...
def samples_plot(X, y, target_names=None, legend=None, fig_multi=True, **kwargs):
